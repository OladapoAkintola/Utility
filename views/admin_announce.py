...
def add_announcement(message): conn = sqlite3.connect("announcements.db") cursor = conn.cursor() cursor.execute("INSERT INTO announcements (message) VALUES (?)", (message,)) conn.commit() conn.close()
...
if st.button("Add Test Announcement"): add_announcement("This is a new test announcement!") st.experimental_rerun()
